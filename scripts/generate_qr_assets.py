...
from web.qr_server import create_shared_form_url, create_signed_form_url, guess_reachable_host
...
def main():
    ...
    parser.add_argument("--shared-role-links", action="store_true", help="Also generate one shared patient URL and one shared provider URL")
    parser.add_argument("--shared-only", action="store_true", help="Generate only shared role URLs (no per-room URLs)")
    ...
    if not args.shared_only:
        ...
        for room in rooms:
            for role in ("patient", "provider"):
                ...
                rows.append({"room_id": room["id"], "room_name": room["name"], "role": role, "url": url, "link_type": "room"})

    if args.shared_role_links or args.shared_only:
        for role in ("patient", "provider"):
            url = create_shared_form_url(base_url, role)
            rows.append({"room_id": "any", "room_name": "All Rooms", "role": role, "url": url, "link_type": "shared"})

    csv_path = output_dir / "qr_urls.csv"
    with csv_path.open("w", encoding="utf-8", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=["room_id", "room_name", "role", "link_type", "url"])
        writer.writeheader()
    ...
    try:
        ...
        for row in rows:
            room_part = row["room_id"]
            filename = f"{row['link_type']}_room_{room_part}_{row['role']}.png"
